# Consists of 2 wired nodes connected to a wireless domain through a base-station node.
# ======================================================================
# Define options
# ======================================================================
...
set opt(nn)             10           ;# 3 number of mobilenodes
...
AddrParams set cluster_num_ $cluster_num
lappend eilastlevel 1 [expr opt(nn)/2] [expr opt(nn)/2 + 1] 		;# number of nodes for each cluster (1 for sink and one for mobile nodes + base station
...
for {set i 0} {$i < $num_wired_nodes} {incr i} {
    # set node($i) [$ns_ node [lindex $temp $i]] 
    set node($i) [$ns node 0.[expr $i + 1].0] 
}
...
$ns_ node-config -wiredRouting OFF
  ...
  for {set j 0} {$j < $opt(nn)} {incr j} {
    set node_($j) [ $ns_ node 1.0.[expr $j + 1] ]
    $node_($j) base-station [AddrParams addr2id [$BS(0) node-addr]]
}

#create links between wired and BS nodes
# Using grid or random topology
$ns_ duplex-link $node(0) $node(1) 5Mb 2ms DropTail
$ns_ duplex-link $node(1) $BS(0) 5Mb 2ms DropTail

$ns_ duplex-link-op $node(0) $node(1) orient down
$ns_ duplex-link-op $node(1) $BS(0) orient left-down
