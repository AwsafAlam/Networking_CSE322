...
set tr /home/ubuntu/ns2\ programs/raw_data/multi_radio_802_15_4_random.tr
...
#CHNG
for {set i 0} {$i < $num_parallel_flow } {incr i} {
     $ns_ at [expr $start_time+$i*$parallel_start_gap+rand()] "$cbr_($i) start"
}
####################################CROSS FLOW
#CHNG
...
#CHNG
for {set i 0} {$i < $num_cross_flow } {incr i} {
	$ns_ at [expr $start_time+$i*$cross_start_gap+rand()] "$cbr_($k) start"
	incr k
}
#######################################################################RANDOM FLOW
set r $k
...
set rt $r
for {set i 1} {$i < [expr $num_random_flow+1]} {incr i} {
	$ns_ at [expr $start_time+rand()] "$cbr_($rt) start"
	incr rt
}
