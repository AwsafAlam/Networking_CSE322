...
/* ******************************************************************
 ...
 FROM VERSION 1.1 of J.F.Kurose
   ...
   - one way network delay averages five time_rdt units (longer if there
...
void A_output(struct msg message)
{
    ...
    if(SEQ_NUM <= win_start + WIN_SIZE){
        ...
        if(buffer.size() < BUFF_SIZE){
            ...
            printf("================ Data Saved in buffer at A ================ \n");
            printPacket(mypkt);
        }
        else{
            printf("================ Buffer full at A [Discard] ================ \n");
...
void A_input(struct pkt packet)
{
    ...
    if(getchecksum(packet) == packet.checksum ){
        ...
        if(packet.acknum >= win_start ){
            ...
            win_start = packet.acknum +1; //sliding window to next seq_no.
            stoptimer(A);
            buffer.pop_back();
            /*TODO: If there are untransmitted packets (in the buffer) with
            sequence numbers that now fall within the window, transmit
            those packet. Start separate timers for each of the packets
            sent.*/


        }
        else{
            printf("================ Duplicate ACK received at A [Retransmit] ================ \n");
            int tmp_ack = packet.acknum;

            for(int i = win_start; i < WIN_SIZE ; i++)
            {
                /* code */
                if(i>= buffer.size())
                    i = (i%buffer.size());
...
{
    printf("\t[A] >> Timeout occurred\n======================= Resending all packets in window ===================\n");
    // Packet corrupted
    starttimer(A,TIMEOUT);
    //if(A_STATE == WAIT_ACK){
...
void B_input(struct pkt packet)
{
    ...
    printPacket(packet);
    if(getchecksum(packet) != packet.checksum){
        printf("================ Packet corrupted found at B [Resend prev ACK] ================ \n");
...
int main(int argc, char const *argv[])
{
    ...
    while (1)
    {
        ...
        if (TRACE >= 2)
        {
            printf("\nEVENT time_rdt: %f,", eventptr->evtime);
        ...
        time_rdt = eventptr->evtime; /* update time_rdt to next event time_rdt */
...
terminate:
    printf(
        " Simulator terminated at time_rdt %f\n after sending %d msgs from layer5\n",
        time_rdt, nsim);
}
...
void init() /* initialize the simulator */
{
    ...
    //scanf("%f",&corruptprob);
    printf("Enter average time_rdt between messages from sender's layer5 [ > 0.0]: %0.2f\n",lambda);
    //scanf("%f",&lambda);
    ...
    time_rdt = 0.0;              /* initialize time_rdt to 0.0 */
...
void insertevent(struct event *p)
{
    ...
    if (TRACE > 2)
    {
        printf("            INSERTEVENT: time_rdt is %lf\n", time_rdt);
        printf("            INSERTEVENT: future time_rdt will be %lf\n", p->evtime);
    }
...
void printevlist(void)
{
    ...
    {
        printf("Event time_rdt: %f, type: %d entity: %d\n", q->evtime, q->evtype,
               q->eventity);
    }
    printf("--------------\n");
}
...
void starttimer(int AorB /* A or B is trying to start timer */, float increment)
{
    ...
    evptr = (struct event *)malloc(sizeof(struct event));
    evptr->evtime = time_rdt + increment;
...
void tolayer3(int AorB, struct pkt packet)
{
    ...
    /* finally, compute the arrival time_rdt of packet at the other end.
       medium can not reorder, so make sure packet arrives between 1 and 10
       time_rdt units after the latest arrival time_rdt of packets
